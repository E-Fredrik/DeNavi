...
export default function EventDetailPage() {
  ...
  if (loading) return <div className="max-w-5xl px-6 lg:px-10 py-12"><p className="text-dash-text-muted" style={{ fontFamily: "var(--font-body)", fontSize: "14px" }}>Memuat...</p></div>;
  if (!event) return <div className="max-w-5xl px-6 lg:px-10 py-12"><p className="text-dash-text-muted" style={{ fontFamily: "var(--font-body)", fontSize: "14px" }}>Acara tidak ditemukan.</p></div>;
  ...
  return (
    <div className="max-w-5xl px-6 lg:px-10 py-8 lg:py-12 relative bg-dash-bg min-h-screen">
      <AnimatePresence>
        {toast && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
            className="fixed top-4 right-4 z-50 px-4 py-3 rounded-lg bg-dash-surface border border-dash-border shadow-lg"
            style={{ color: toast.ok ? "#10b981" : "#ef4444", fontFamily: "var(--font-body)", fontWeight: 500, fontSize: "13px" }}>
            {toast.msg}
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
        <Link href="/admin/dashboard/events" className="inline-flex items-center gap-2 mb-6 hover:opacity-70 transition-opacity">
          ...
          <span className="text-dash-text-muted" style={{ fontFamily: "var(--font-body)", fontWeight: 400, fontSize: "13px" }}>Kembali ke daftar acara</span>
        </Link>
        <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
          <div>
            <h1 className="text-dash-text" style={{ fontFamily: "var(--font-body)", fontWeight: 700, fontSize: "24px", letterSpacing: "-0.02em" }}>{event.name}</h1>
            <p className="mt-1 text-dash-text-sub" style={{ fontFamily: "var(--font-body)", fontWeight: 400, fontSize: "13px" }}>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <button onClick={() => setShowAddGuest(true)}
              className="flex items-center gap-2 px-3.5 py-2 rounded-lg transition-colors hover:bg-dash-surface-hover bg-dash-surface border border-dash-border text-dash-text"
            ...
            <button 
              ...
              className={`flex items-center gap-2 px-3.5 py-2 rounded-lg transition-colors border ${hasEmailBuilder ? "bg-dash-surface border-dash-border text-dash-text hover:bg-dash-surface-hover" : "bg-dash-surface-alt/50 border-dash-border/50 text-dash-text-muted cursor-not-allowed opacity-80"}`}
              style={{ fontFamily: "var(--font-body)", fontWeight: 500, fontSize: "12px" }}>
              <LayoutTemplate className="w-3.5 h-3.5" strokeWidth={1.5} /> 
              Pembuat Email
              {!hasEmailBuilder && <Lock className="w-3 h-3 ml-1 text-dash-text-muted" strokeWidth={1.5} />}
            </button>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3 mt-6">
        {stats.map((s) => (
          <div key={s.label} className="p-4 rounded-xl bg-dash-surface border border-dash-border">
            <span className="text-dash-text-muted" style={{ fontFamily: "var(--font-body)", fontWeight: 400, fontSize: "11px", letterSpacing: "0.04em", textTransform: "uppercase" }}>{s.label}</span>
            <div className="mt-1 text-dash-text" style={{ fontFamily: "var(--font-body)", fontWeight: 700, fontSize: "20px", letterSpacing: "-0.02em" }}>{s.value}</div>
          </div>
        ))}
      </div>

      {/* Progress */}
      <div className="mt-4 h-1 rounded-full overflow-hidden bg-dash-surface-alt">
        <div className="h-full rounded-full transition-all duration-500" style={{ width: `${total > 0 ? (checkedIn / total) * 100 : 0}%`, background: "var(--dash-accent)" }} />
      </div>

      {/* Mode toggle */}
      <div className="mt-8">
        <div className="flex items-center gap-1 p-1 rounded-lg w-fit bg-dash-surface border border-dash-border">
          {(["search", "scan"] as Mode[]).map((m) => (
            <button key={m} onClick={() => setMode(m)}
              className={`flex items-center gap-2 px-4 py-2 rounded-md transition-colors ${mode === m ? "bg-dash-surface-alt border border-dash-border" : "border border-transparent"}`}
              style={{ fontFamily: "var(--font-body)", fontWeight: mode === m ? 500 : 400, fontSize: "12px" }}>
              {m === "search" ? <Search className={`w-3.5 h-3.5 ${mode === m ? "text-dash-text" : "text-dash-text-muted"}`} strokeWidth={1.5} /> : <QrCode className={`w-3.5 h-3.5 ${mode === m ? "text-dash-text" : "text-dash-text-muted"}`} strokeWidth={1.5} />}
              <span className={mode === m ? "text-dash-text" : "text-dash-text-muted"}>{m === "search" ? "Pencarian" : "Pindai QR"}</span>
            </button>
          ))}
        </div>
        ...
        {mode === "scan" && (
          <div className="mt-4 flex flex-col gap-4">
            ...
            <div>
              <p className="text-dash-text-muted mb-2" style={{ fontFamily: "var(--font-body)", fontSize: "11px", textTransform: "uppercase", letterSpacing: "0.04em" }}>Atau masukkan kode secara manual</p>
              <div className="flex gap-2">
                <input value={qrInput} onChange={(e) => setQrInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleQrScan()}
                  ...
                  className="flex-1 px-4 py-3 rounded-lg outline-none transition-colors bg-dash-surface border border-dash-border text-dash-text"
                  style={{ fontFamily: "var(--font-body)", fontSize: "14px" }} />
                <button onClick={handleQrScan} className="px-5 py-3 rounded-lg hover:opacity-90 transition-opacity" style={{ background: "var(--dash-accent)", fontFamily: "var(--font-body)", fontWeight: 500, fontSize: "13px", color: "#fff" }}>Verifikasi</button>
              </div>
            </div>
          </div>
        )}
        {mode === "search" && (
          <div className="mt-4 flex items-center gap-2 px-4 py-3 rounded-lg bg-dash-surface border border-dash-border">
            <Search className="w-4 h-4 flex-shrink-0 text-dash-text-muted" strokeWidth={1.5} />
            <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Cari nama tamu..."
              className="flex-1 bg-transparent outline-none text-dash-text" style={{ fontFamily: "var(--font-body)", fontSize: "14px" }} />
            {query && <button onClick={() => setQuery("")}><X className="w-3.5 h-3.5 text-dash-text-muted" strokeWidth={1.5} /></button>}
          </div>
        )}
      </div>

      {/* Guest list */}
      <div className="mt-4 rounded-xl overflow-hidden border border-dash-border">
        <div className="max-h-[480px] overflow-y-auto">
          {filtered.map((guest, idx) => (
            <div key={guest.id}
              className={`flex items-center justify-between px-4 sm:px-5 py-3.5 transition-colors hover:bg-dash-surface-hover bg-dash-surface ${idx < filtered.length - 1 ? "border-b border-dash-border" : ""}`}>
              <div className="flex items-center gap-3 min-w-0">
                <div className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 bg-dash-surface-alt border border-dash-border">
                  <span className="text-dash-text" style={{ fontFamily: "var(--font-body)", fontWeight: 500, fontSize: "11px" }}>{guest.firstName.charAt(0)}</span>
                </div>
                <div className="min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className={`${guest.hasCheckedIn ? "text-dash-text-muted" : "text-dash-text"}`} style={{ fontFamily: "var(--font-body)", fontWeight: 500, fontSize: "13px" }}>{getFullName(guest)}</span>
                    {guest.partySize > 1 && (
                      <span className="px-1.5 py-0.5 rounded flex-shrink-0 bg-dash-surface-alt border border-dash-border text-dash-text-sub"
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className="text-dash-text-muted" style={{ fontFamily: "monospace", fontWeight: 400, fontSize: "10px" }}>{guest.qrTicket}</span>
                    {guest.tableNumber && (
                      <span className="text-dash-text-sub" style={{ fontFamily: "var(--font-body)", fontWeight: 400, fontSize: "10px" }}>
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <button onClick={() => setInviteModal({ guest })}
                  className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-lg transition-colors bg-dash-surface-alt hover:bg-dash-surface-hover text-dash-text"
                  style={{ fontFamily: "var(--font-body)", fontWeight: 500, fontSize: "11px" }}>
                  <Send className="w-3 h-3" strokeWidth={1.5} /> Undang
                </button>
                <button onClick={() => handleAddPlusOne(guest)}
                  className="px-2 py-1 rounded text-dash-text hover:bg-dash-surface-hover transition-colors border border-dash-border"
                  style={{ fontFamily: "var(--font-body)", fontWeight: 500, fontSize: "10px" }}>+1</button>
                <button onClick={() => handleEditGuest(guest)}
                  className="px-2 py-1 rounded text-dash-text-muted hover:bg-dash-surface-hover transition-colors"
                  style={{ fontFamily: "var(--font-body)", fontSize: "10px" }}>Edit</button>
                {guest.hasCheckedIn ? (
                  <button onClick={() => handleEditGuest(guest)} className="flex items-center gap-1 px-2 py-1 rounded bg-dash-surface-alt border border-dash-accent-light hover:bg-dash-surface-hover dark:hover:bg-dash-surface-hover transition-colors">
                    <Check className="w-3 h-3 text-dash-accent-light" strokeWidth={2} />
                    <span className="text-dash-accent-light" style={{ fontFamily: "var(--font-body)", fontWeight: 500, fontSize: "10px" }}>HADIR ({guest.actualAttendees || guest.partySize})</span>
                  </button>
                ) : (
                  <button onClick={() => handleCheckIn(guest)}
                    className="px-3 py-1.5 rounded-lg hover:opacity-90 transition-opacity"
                    style={{ background: "var(--dash-accent)", fontFamily: "var(--font-body)", fontWeight: 500, fontSize: "12px", color: "#fff" }}>Check In</button>
                )}
              </div>
            </div>
          ))}
          {filtered.length === 0 && (
            <div className="px-5 py-8 text-center bg-dash-surface">
      ...
      {/* Export Modal */}
      {showExport && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/20 dark:bg-black/60">
          <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
            className="w-full max-w-sm rounded-xl p-6 bg-dash-surface border border-dash-border shadow-2xl">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-dash-text" style={{ fontFamily: "var(--font-body)", fontWeight: 600, fontSize: "15px" }}>Ekspor Daftar Tamu</h3>
              <button onClick={() => setShowExport(false)} className="p-1 hover:opacity-70"><X className="w-4 h-4 text-dash-text-muted" strokeWidth={1.5} /></button>
            </div>
            <p className="text-dash-text-sub -mt-3 mb-5" style={{ fontFamily: "var(--font-body)", fontWeight: 400, fontSize: "12px" }}>Pilih format untuk mengunduh daftar tamu.</p>
            
            <div className="flex flex-col gap-3">
              <button onClick={() => handleExport("csv")}
                className="w-full py-2.5 rounded-lg transition-colors border bg-dash-surface-alt border-dash-border text-dash-text hover:bg-dash-accent hover:text-dash-on-accent hover:bg-dash-accent hover:border-dash-accent-light"
      ...
      {showAddGuest && (
        <div className="fixed inset-0 z-50 flex items-start justify-center p-4 sm:p-6 overflow-y-auto" style={{ background: "rgba(0,0,0,0.6)" }}>
          <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
            className="w-full max-w-sm rounded-xl p-6 bg-dash-surface border border-dash-border my-auto">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-dash-text" style={{ fontFamily: "var(--font-body)", fontWeight: 600, fontSize: "15px" }}>Tambah Tamu</h3>
              <button onClick={() => setShowAddGuest(false)} className="p-1 hover:opacity-70"><X className="w-4 h-4 text-dash-text-muted" strokeWidth={1.5} /></button>
            </div>
            <p className="text-dash-text-sub -mt-3 mb-2" style={{ fontFamily: "var(--font-body)", fontWeight: 400, fontSize: "12px" }}>Tambahkan undangan utama. Gunakan tombol <strong>+1</strong> untuk menambah pengikut.</p>
            <div className="flex flex-col gap-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block mb-1.5 text-dash-text-muted" style={{ fontFamily: "var(--font-body)", fontWeight: 500, fontSize: "12px", letterSpacing: "0.04em", textTransform: "uppercase" }}>Nama Depan</label>
                  <input value={newGuestFirstName} onChange={(e) => setNewGuestFirstName(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleAddGuest()} placeholder="Budi" autoFocus
                    className="w-full px-3.5 py-2.5 rounded-lg outline-none transition-colors bg-dash-surface-alt border border-dash-border text-dash-text"
                    style={{ fontFamily: "var(--font-body)", fontSize: "14px" }} />
                </div>
                <div>
                  <label className="block mb-1.5 text-dash-text-muted" style={{ fontFamily: "var(--font-body)", fontWeight: 500, fontSize: "12px", letterSpacing: "0.04em", textTransform: "uppercase" }}>Nama Belakang</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block mb-1.5 text-dash-text-muted" style={{ fontFamily: "var(--font-body)", fontWeight: 500, fontSize: "12px", letterSpacing: "0.04em", textTransform: "uppercase" }}>Jumlah Orang</label>
                ...
                <div>
                  <label className="block mb-1.5 text-dash-text-muted" style={{ fontFamily: "var(--font-body)", fontWeight: 500, fontSize: "12px", letterSpacing: "0.04em", textTransform: "uppercase" }}>Nomor WhatsApp</label>
              ...
              <div>
                <label className="block mb-1.5 text-dash-text-muted" style={{ fontFamily: "var(--font-body)", fontWeight: 500, fontSize: "12px", letterSpacing: "0.04em", textTransform: "uppercase" }}>Opsi Tempat Duduk</label>
                <select value={newSeatingOption} onChange={(e) => setNewSeatingOption(e.target.value as SeatingOption)}
                  className="w-full px-3.5 py-2.5 mb-3 rounded-lg outline-none transition-colors bg-dash-surface-alt border border-dash-border text-dash-text"
                  style={{ fontFamily: "var(--font-body)", fontSize: "14px" }}>
                ...
                {newSeatingOption !== "anywhere" && (
                  event.venueLayoutConfig ? (
                    <div className="mt-2 mb-4 bg-dash-surface-alt rounded-xl border border-dash-border p-4 overflow-auto relative max-h-[40vh]">
                      <p className="text-dash-text-muted mb-4" style={{ fontFamily: "var(--font-body)", fontSize: "12px" }}>
                      ...
                      />
                      {(newTable || newSeat) && (
                        <div className="mt-4 p-3 bg-dash-surface rounded-lg border border-dash-border flex justify-between items-center sticky bottom-0 z-10">
                    ...
                    <div className={`grid gap-3 ${newSeatingOption === "preset" ? "grid-cols-2" : "grid-cols-1"}`}>
                      <div>
                        <label className="block mb-1.5 text-dash-text-muted" style={{ fontFamily: "var(--font-body)", fontWeight: 500, fontSize: "12px", letterSpacing: "0.04em", textTransform: "uppercase" }}>Meja</label>
                      ...
                      {newSeatingOption === "preset" && (
                        <div>
                          <label className="block mb-1.5 text-dash-text-muted" style={{ fontFamily: "var(--font-body)", fontWeight: 500, fontSize: "12px", letterSpacing: "0.04em", textTransform: "uppercase" }}>Kursi</label>
              ...
              <button onClick={handleAddGuest} disabled={!newGuestFirstName.trim()}
                ...
                style={{ background: "var(--dash-accent)", fontFamily: "var(--font-body)", fontWeight: 500, fontSize: "13px", color: "#fff" }}>Simpan Tamu</button>
            </div>
          </motion.div>
        </div>
      )}
      ...
      {editingGuest && (
        <div className="fixed inset-0 z-50 flex items-start justify-center p-4 sm:p-6 overflow-y-auto" style={{ background: "rgba(0,0,0,0.6)" }}>
          <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
            className="w-full max-w-sm rounded-xl p-6 bg-dash-surface border border-dash-border my-auto">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-dash-text" style={{ fontFamily: "var(--font-body)", fontWeight: 600, fontSize: "15px" }}>Edit: {getFullName(editingGuest)}</h3>
              <button onClick={() => setEditingGuest(null)} className="p-1 hover:opacity-70"><X className="w-4 h-4 text-dash-text-muted" strokeWidth={1.5} /></button>
            </div>
            <div className="flex flex-col gap-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="number" value={editPartySize} onChange={(e) => setEditPartySize(e.target.value === "" ? "" : Number(e.target.value))} onBlur={() => setEditPartySize(prev => { const n = Number(prev); return isNaN(n) || n < 1 ? 1 : n; })} min={1}
                    className="w-full px-3.5 py-2.5 rounded-lg outline-none bg-dash-surface-alt border border-dash-border text-dash-text"
                    style={{ fontFamily: "var(--font-body)", fontSize: "14px" }} />
                </div>
                <div>
                  <label className="block mb-1.5 text-dash-text-muted" style={{ fontFamily: "var(--font-body)", fontWeight: 500, fontSize: "12px", letterSpacing: "0.04em", textTransform: "uppercase" }}>Nomor WhatsApp</label>
              ...
              <div>
                ...
                <select value={editSeatingOption} onChange={(e) => setEditSeatingOption(e.target.value as SeatingOption)}
                  className="w-full px-3.5 py-2.5 mb-3 rounded-lg outline-none bg-dash-surface-alt border border-dash-border text-dash-text"
                  style={{ fontFamily: "var(--font-body)", fontSize: "14px" }}>
                  <option value="anywhere">Bebas Pilih Tempat (Self-Select Anywhere)</option>
                  <option value="specific_table">Pilih Kursi di Meja Tertentu</option>
                  <option value="preset">Sudah Ditentukan (Preset Seating)</option>
              ...
              <div className="grid grid-cols-2 gap-3 mt-1">
                <label className="flex items-center gap-2 text-dash-text-sub cursor-pointer" style={{ fontFamily: "var(--font-body)", fontWeight: 500, fontSize: "13px" }}>
                  <input type="checkbox" checked={editHasCheckedIn} onChange={(e) => setEditHasCheckedIn(e.target.checked)} className="w-4 h-4 rounded border-dash-border" />
                  Sudah Check-In
                </label>
                {editHasCheckedIn && (
                  <div>
                    <label className="block mb-1.5 text-dash-text-muted" style={{ fontFamily: "var(--font-body)", fontWeight: 500, fontSize: "12px", letterSpacing: "0.04em", textTransform: "uppercase" }}>Jumlah Hadir</label>
              ...
              <div className="flex items-center gap-2 mt-2">
                <button onClick={() => handleDeleteGuest(editingGuest.id)}
                  className="px-4 py-2.5 rounded-lg hover:bg-dash-surface-hover text-[#ff6b7a] transition-colors"
                ...
                <button onClick={handleUpdateGuest}
                  ...
                  style={{ background: "var(--dash-accent)", fontFamily: "var(--font-body)", fontWeight: 500, fontSize: "13px", color: "#fff" }}>Simpan Perubahan</button>
              </div>
            </div>
          </motion.div>
        </div>
      ...
      {checkingInGuest && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.6)" }}>
          <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
            className="w-full max-w-sm rounded-xl p-6 bg-dash-surface border border-dash-border">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-dash-text" style={{ fontFamily: "var(--font-body)", fontWeight: 600, fontSize: "15px" }}>
                Check-in: {getFullName(checkingInGuest)}
              </h3>
              <button onClick={() => setCheckingInGuest(null)} className="p-1 hover:opacity-70"><X className="w-4 h-4 text-dash-text-muted" strokeWidth={1.5} /></button>
            </div>
            
            {checkingInGuest.partySize > 1 && (
            <p className="text-dash-text-sub mb-5" style={{ fontFamily: "var(--font-body)", fontWeight: 400, fontSize: "13px" }}>
            ...
            <div className="flex flex-col gap-4">
              ...
              {checkingInGuest.partySize > 1 && (
              <div>
                <label className="block mb-1.5 text-dash-text-muted" style={{ fontFamily: "var(--font-body)", fontWeight: 500, fontSize: "12px", letterSpacing: "0.04em", textTransform: "uppercase" }}>Tamu yang Hadir</label>
              ...
              {hasAngpao && (
              <div className="flex flex-col gap-3 p-3 rounded-lg bg-dash-surface-alt border border-dash-border">
                <span className="text-dash-text-sub" style={{ fontFamily: "var(--font-body)", fontWeight: 600, fontSize: "12px" }}>Terima Angpao / Hadiah</span>
                <div>
                  <label className="block mb-1.5 text-dash-text-muted" style={{ fontFamily: "var(--font-body)", fontWeight: 500, fontSize: "11px", letterSpacing: "0.04em", textTransform: "uppercase" }}>Nominal (Rp)</label>
                  <input type="number" value={angpaoAmount} onChange={(e) => setAngpaoAmount(e.target.value === "" ? "" : Number(e.target.value))} min={0} placeholder="Contoh: 500000"
                    className="w-full px-3.5 py-2 rounded-lg outline-none bg-dash-surface border border-dash-border text-dash-text"
                    style={{ fontFamily: "var(--font-body)", fontSize: "13px" }} />
                </div>
                <div>
                  <label className="block mb-1.5 text-dash-text-muted" style={{ fontFamily: "var(--font-body)", fontWeight: 500, fontSize: "11px", letterSpacing: "0.04em", textTransform: "uppercase" }}>Deskripsi Hadiah</label>
              ...
              <button onClick={() => confirmCheckIn(checkingInGuest.id, checkingInGuest.partySize > 1 ? Number(checkInAttendees) || 1 : 1)}
                ...
                style={{ background: "var(--dash-accent)", fontFamily: "var(--font-body)", fontWeight: 500, fontSize: "13px", color: "#fff" }}>
                Konfirmasi Check-In
              </button>
            </div>
          </motion.div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.6)" }}>
          <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => setInviteModal(null)} className="p-1 hover:opacity-70"><X className="w-4 h-4 text-dash-text-muted" strokeWidth={1.5} /></button>
            </div>
            <p className="text-dash-text-sub mb-5" style={{ fontFamily: "var(--font-body)", fontWeight: 400, fontSize: "13px" }}>
            ...
            <div className="flex flex-col gap-4">
              <div>
                <label className="block mb-1.5 text-dash-text-muted" style={{ fontFamily: "var(--font-body)", fontWeight: 500, fontSize: "12px", letterSpacing: "0.04em", textTransform: "uppercase" }}>Alamat Email</label>
      ...
      {showSecurity && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.6)" }}>
          <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
            className="w-full max-w-sm rounded-xl p-6 bg-dash-surface border border-dash-border">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-dash-text" style={{ fontFamily: "var(--font-body)", fontWeight: 600, fontSize: "15px" }}>Keamanan Staf</h3>
              <button onClick={() => setShowSecurity(false)} className="p-1 hover:opacity-70"><X className="w-4 h-4 text-dash-text-muted" strokeWidth={1.5} /></button>
            </div>
            <p className="text-dash-text-sub -mt-3 mb-5" style={{ fontFamily: "var(--font-body)", fontWeight: 400, fontSize: "12px" }}>Atur kata sandi agar staf bisa mengakses halaman check-in untuk acara ini.</p>
            
            <div className="flex flex-col gap-4">
              <div>
                <label className="block mb-1.5 text-dash-text-muted" style={{ fontFamily: "var(--font-body)", fontWeight: 500, fontSize: "12px", letterSpacing: "0.04em", textTransform: "uppercase" }}>Kata Sandi Check-in</label>
                <div className="relative">
                  <input type={showPassword ? "text" : "password"} value={newPassword} onChange={(e) => setNewPassword(e.target.value)} 
                    placeholder="Contoh: rahasia123"
                    className="w-full px-3.5 py-2.5 rounded-lg outline-none transition-colors bg-dash-surface-alt border border-dash-border text-dash-text"
                    style={{ fontFamily: "var(--font-body)", fontSize: "14px" }} />
                  <button onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-dash-text-muted hover:text-dash-text-sub transition-colors">
                    {showPassword ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                  </button>
                </div>
                <p className="mt-2 text-dash-text-muted" style={{ fontFamily: "var(--font-body)", fontSize: "10px" }}>Kosongkan untuk menonaktifkan perlindungan kata sandi pada acara ini.</p>
              </div>
